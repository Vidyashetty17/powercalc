...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Save calculation to backend and display
  btnSaveCalc.addEventListener('click', () => {
    const rate = parseFloat(rateInput.value);
    if(selectedAppliances.length === 0 || isNaN(rate)) return;
    const calcData = calculateEstimate();
  ...
  function renderHistory() {
    ...
    calculationHistory.forEach(item => {
      const div = document.createElement('div');
      div.className = 'history-item';
      const dateStr = new Date(item.timestamp || item.date).toLocaleDateString('en-IN', {
        year: 'numeric', month: 'long', day: 'numeric'
      });
      div.innerHTML = `
        <div class="history-item-header">
